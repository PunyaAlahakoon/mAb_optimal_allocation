...
dat2=data.frame("risk_infection"=risks[,1],"risk_hos"=risks[,2],"avoided_hos"=NA,"doses"=N,"hos_per_dose"=NA,"method"=rep("Prophylaxis only"))
...
for (i in 1:q) {
  r_b_i=risks[i,1]
  r_b_h=risks[i,2]
  out_2=get_hos_time_pp4(N,tspan,drug_number,r_b_i,r_b_h,pp,others)
  #out_2=get_hos_no_deacy2(N,drug_number,r_b_i,r_b_h,pp,others)
  hos_dat2=c(hos_dat2,out_2[1])
}
...
p1<-ggplot(all_dat,aes(x=risk_infection,y=hos_per_dose,color=as.factor(risk_hos)))+
  geom_line()+
  facet_wrap(~method)+
  ...
  labs(color="Risk of hospitalisation")+ 
...
ind_x=min(which(dat3$hos_per_dose>=1))
xx=dat3$risk_infection[ind_x]

p2<-ggplot(dat3,aes(x=risk_infection,y=hos_per_dose,color=as.factor(risk_hos)))+
  geom_line()+
  facet_wrap(~method)+
  ylab("Hospitalisations avoided per dose")+
  xlab("Risk of infection")+
...
ggsave("fig_s2.png",plot,height = 10,width = 12)

#plot

p1<-ggplot(all_dat,aes(x=risk_hos,y=hos_per_dose,color=as.factor(risk_infection)))+
  geom_line()+
  facet_wrap(~method)+
  ylab("Hospitalisations avoided per dose")+
  xlab("Risk of hospitalisation")+
...
p2<-ggplot(dat3,aes(x=risk_hos,y=hos_per_dose,color=as.factor(risk_infection)))+
  geom_line()+
  facet_wrap(~method)+
  ylab("Hospitalisations avoided per dose")+
  xlab("Risk of hospitalisation")+
...
plot
#plot<-grid.arrange(p1,p2,ncol = 1,heights=c(1,1))
ggsave("fig_s3.png",plot,height = 10,width = 12)
...
plot
#plot<-grid.arrange(p1,p2,ncol = 1,heights=c(1,1))
ggsave("cose_efficieny_avoided_hos.png",plot,height = 12,width = 10)
