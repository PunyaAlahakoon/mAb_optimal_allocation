...
r_bi=seq(0,1,by=0.01)

#risk of hospitalisations
r_bh=seq(0,1,by=0.01)
...
#combinations of maximal efficacy:
prohy_max_inf_eff=c(0.9443,0.9443,0.25,0.25) #prohylaxis from progression to infection 
prohy_max_hos_eff=c(0.87,0.87,0.25,0.25) #prophylaxis from progression to hospitalisation 
ther_max_eff=c(0.67,0.25,0.67,0.25) #therpautic from progression to hospitalisation 


plot_type=c("Evusheld for the ancestral variant","Efficacy for therapeutic = 25%","Efficacy for prohylaxis = 25%","Efficacy for prohylaxis and therapeutic = 25%")
tspan=seq(0,179,by=1)
n=length(pp)
comb=expand.grid(av_doses,pp,r_bi,r_bh)

all_sub_dats=data.frame() #store data with different efficacy combinations 
...
for (i in 1:nq) {
  ...
  out=get_hos_time_pp_ther_prohy2(N,tspan,drug_number,r_b_i,r_b_h,av_d,p_p,prohy_max_hos_eff[k],prohy_max_inf_eff[k],ther_max_eff[k]) 
...
sub_dat=subset(new_dat,availble_doses==500)

#ind_x=min(which(sub_dat$optimal_prohylaxis_dose==1))
#xx=sub_dat$risk_infection[ind_x]


sub_dat0=data.frame(sub_dat,"eff"=plot_type[k])

#repeat all the previous lines by changing the 
#sub_dat1=data.frame(sub_dat,"eff"=rep("Efficacy for theraputic = 25%"))

#sub_dat2=data.frame(sub_dat,"eff"=rep("Efficacy for prohylaxis = 25%"))
#sub_dat3=data.frame(sub_dat,"eff"=rep("Efficacy for prohylaxis and theraputic = 25%"))
all_sub_dats=rbind(all_sub_dats,sub_dat0)
}


#all_sub_dats=rbind(sub_dat0,sub_dat1,sub_dat2,sub_dat3)




xs=data.frame("val"=c(0.7,0.26,0.5,0.67),"eff"=plot_type)
all_sub_dats$eff<-factor(all_sub_dats$eff,levels =plot_type)
xs$eff<-factor(xs$eff,levels =plot_type)

p=ggplot(all_sub_dats, aes(x=risk_infection,y=risk_hos,fill=max_avoided_hos_per_dose)) + 
  geom_tile()+
  scale_fill_viridis()+
  #  geom_text(aes(label=optimal_prohylaxis_dose),size=2)+
  geom_vline(data=xs,aes(xintercept = as.numeric(val)),linetype="twodash")+
  # geom_text(data=xs,aes(x=0.025,y=0.25,label="text"))+
  facet_wrap(~eff,ncol = 2)+
  ...
  #ggtitle("Efficacy for prohylaxis and theraputic= 25%")+
...
ggsave("fig_C.png",last_plot(),height = 6,width = 10)
