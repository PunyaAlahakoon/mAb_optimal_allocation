...
load("data/eff_est_data_mAbs.RData")



all_dat=data.frame("availble_doses"=NA,"risk_infection"=NA,
                   "risk_hos"=NA,"optimal_prohylaxis_dose"=NA,"max_avoided_hos_per_dose"=NA,"total_theraputic_administered"=NA,
                   "hos_avoded"=NA,"day_of_treatment"=NA,"theraputic_eff"=NA)
...
for(k in 1:8){
  ...
  eff_theraputic=eff_est_data_mAbs[k,2]*0.01
  ...
  for (i in 1:nq) {
    ...
    out=get_hos_time_pp_ther_prohy2(N,tspan,drug_number,r_b_i,r_b_h,av_d,p_p,prohy_max_hos_eff=0.87,prohy_max_inf_eff=0.9443,eff_theraputic)
  ...
  new_dat=data.frame("availble_doses"=rep(av_doses,each=q),"risk_infection"=rep(risks[,1]),
                     ...
                     "hos_avoded"=NA,"day_of_treatment"=rep(paste("Day ",eff_est_data_mAbs[k,1])),"theraputic_eff"=rep(eff_est_data_mAbs[k,2]))
  ...
  ind_x=min(which(new_dat$optimal_prohylaxis_dose==1))
  xx3=new_dat$risk_infection[ind_x]
  xx=c(xx,xx3)
}


all_dat=all_dat[-1,] #remove the first row of NAs 

all_xx=data.frame("day_of_treatment"=(paste("Day ",eff_est_data_mAbs[1:8,1])),xx)

all_xx$prohy<-1-xx
ax<-melt(all_xx)
ax$variable<-factor(ax$variable,levels=c("prohy","xx"))

ggplot(data=all_xx, aes(x=day_of_treatment, y=xx)) +
  geom_point(shape=3,size=5,color="#7570B3",  stroke = 2)+
  #geom_bar(stat="identity",color="#1B9E77",fill='#1B9E77',size=1,alpha=0.5)+
  ylab("Risk of infection threshold which \n optimal strategy starts switching \n from therapeutic to prohylaxis")+
  xlab("Day of treatment")+
  ylim(0,1)+
  #  scale_fill_manual(values =c("#1B9E77","#7570B3"),labels=c("Prophylaxis","Therapeutic"))+
  theme_minimal(12)+
