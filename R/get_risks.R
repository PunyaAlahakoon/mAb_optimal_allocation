...
get_risks<-function(variant_number,times,prohy_max_hos_eff,prohy_max_inf_eff){
  ...
  for (i in 1:3) {
    ...
    #get the abs in mg/L:
    dose_m=ab_vs_time_curve2(times,ab0[i],half_t[i]) #exponential decay curev, give  the antibody amount
    ...
    e_t=get_prophy_hos_curve(dose_f,prohy_max_hos_eff) #efficacy

    #risk from exposure to infections
    #dose_i=dose_m/fact #fold IC50 inputs

    e_p=get_prophy_inf_curve(dose_f,prohy_max_inf_eff) #efficacy
    e_exp_inf[,i]=1-e_p #riskfrom exposure to infection
  ...
  #plot(times,e_h)
  colnames(e_exp_inf)=colnames(e_hos)=c("Adintrevimab", "Casirivimab/imdevimab", "Tixagevimab/cilgavimab")

  e_exp_inf=data.frame(e_exp_inf,"time"=times,"dose"=dose_f)
  e_hos=data.frame(e_hos,"time"=times,"dose"=dose_f)

 # e_exp_inf
  #e_hos

  output=list(e_exp_inf,e_hos)
}
